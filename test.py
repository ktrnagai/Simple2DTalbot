...
def CalcObject(n, xlambda, l_micron):
    ...
    OBJ = np.where((xx**2.0+yy**2.0)<Dim**2.0, sp.sqrt(Dim**2.0-(xx**2.0+yy**2.0))*ObjPhase*2.0, np.zeros((n,n)))
    return OBJ

def SPFunc(n, l_micron):
...
Phs = np.imag(OBJ)*5.0/2.0*3.0 # 第三タルボ位置の計算
...
Moire = Abs*(1.0-0.5*np.cos(xx*2.0*pi/period-DPy))*(1.0-0.5*np.cos(yy*2.0*pi/period-DPx))/4.0
Ref = (1.0-0.5*np.cos(xx*2.0*pi/period))*(1.0-0.5*np.cos(yy*2.0*pi/period))/4.0

SPF = SPFunc(n, l_micron)
